import { CircleHelp, Github, RadioTower, ShieldCheck } from "lucide-react";
...
export function HelpPanel({ onOpenIssues }: HelpPanelProps) {
  ...
  return <section className="help-view">
    <header className="topbar help-heading">
      ...
      <button type="button" className="secondary-action" onClick={onOpenIssues}><Github size={17} />{t.help.openIssue}</button>
    </header>
    <div className="help-grid">
      ...
      <article className="faq-card faq-support"><div><h2>{t.help.moreTitle}</h2><p>{t.help.moreBody}</p></div><button type="button" className="primary-action" onClick={onOpenIssues}><Github size={17} />{t.help.openIssue}</button></article>
    </div>
  </section>;
}
